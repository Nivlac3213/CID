...
StreamServerCore::StreamServerCore(int port) 
    ...
    std::thread(&SignalProcessor::processData, &signalProcessor).detach();
}
        ...
        try {
            ...
            try
            {
                ...
                if (signalDetected){
                    ...
                    // Create PiData struct
                    PiData data;
                    data.timestamp = signalProcessor.getTime();// request.id;
                    data.samples = dataVector; //vectorizeSamples(request.data); // Assuming this function exists and converts raw data to a vector

                    // Determine Pi ID based on IP or other identifier
                    // int piId = getPiIdFromIP(clientIP);
                    int piId = 0;
                    if (clientIP == "172.19.172.171") {
                        //std::cout << "Signal detected on 172.178.0.171" << std::endl;
                        piId = 0;
                    } else if (clientIP == "172.19.172.72") {
                            //std::cout << "Signal detected on 172.178.0.72" << std::endl;
                            piId = 1;
                    } else if (clientIP == "172.19.172.252") {
                        //std::cout << "Signal detected on 172.178.0.252" << std::endl;
                        piId = 2;
                    }

                    // Add data to SignalProcessor
//                    int check = 0;
//                    check = signalProcessor.addPiData(piId, data);
//                    if (check == 1) {
//                        std::raise(SIGINT);
//                    }

                    signalProcessor.addPiData(piId, data);
                }
//                else {
//                    std::cout << std::flush;
